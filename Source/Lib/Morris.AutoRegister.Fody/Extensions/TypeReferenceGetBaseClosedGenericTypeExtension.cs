...
internal static class TypeDefinitionGetBaseClosedGenericTypeExtension
{
	...
	{
		TypeDefinition baseTypeDefinition = baseType.Resolve();

		if (!baseTypeDefinition.HasGenericParameters)
			return baseType;

		TypeReference[] types =
			baseTypeDefinition.IsInterface
		...
		TypeReference[] parameters = baseTypeDefinition
			.GenericParameters
	...
	{
		return FindInterfacePathRecursive(descendantInterface, baseInterface)
			.Reverse()
			.ToArray();
	}

	private static IEnumerable<TypeReference> FindInterfacePathRecursive(
		TypeReference descendantInterface,
		TypeReference baseInterface)
	{
		if (descendantInterface.IsSameAs(baseInterface))
			return [descendantInterface];

		IEnumerable<TypeReference> interfaces = descendantInterface
			.Resolve()
			.Interfaces
			.Select(x => x.InterfaceType);

		IEnumerable<TypeReference>? childRoute = interfaces
